...
*   **Preferred Concise Styling for Controller Actions:**

    When the success and failure outcomes primarily involve redirection with a message, the following concise style is highly recommended. This keeps controller actions lean and focused on orchestrating the workflow and response.

    ```ruby
    # app/controllers/organizations/invitations_controller.rb (Example)
    # (Ensure Pundit is used for authorization, e.g., `authorize @organization, :invite_member?` or similar)
    # (Service parameters like `invite_params` should be defined in a private method)

    def create
      authorize current_user, :invite_member? # Or appropriate authorization for the action

      # invite_params would typically be defined in a private method:
      # def invite_params
      #   params.require(:invitation).permit(:email, :role).merge(
      #     inviter: current_user,
      #     organization: current_organization # Assuming current_organization is available
      #   )
      # end
    ...
    ```
    This example demonstrates the ideal structure:
    *   Authorization is handled before the service call.
    *   Service parameters are prepared (ideally in a private method abstracting `params` access).
    *   The service is called with a block.
    *   `on.success` and `on.failure` handlers are single-line blocks using `{ ... }` for brevity when they perform a simple action like redirection.
    *   The payload from the service (e.g., `payload[:message]`, `failure_payload[:message]`) is used directly in the response.

    For more complex scenarios where different types of failures need distinct handling (e.g., rendering a form with specific errors vs. redirecting), or when success requires more than just a redirect, the more detailed pattern shown below can be used.
